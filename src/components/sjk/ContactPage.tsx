      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(90vh-120px)] max-w-[1400px] flex-col justify-end px-8 pb-20">
        <div className="max-w-3xl">
          ...
          <h1 className="text-[64px] font-medium leading-[0.95] tracking-tight md:text-[96px]">
            Kết nối cùng{" "}
            <span className="text-[var(--brand-orange)]">SJK Group</span>
          </h1>
...
function ContactQuickInfo() {
  return (
    <section className="border-t border-[var(--charcoal)]/10 bg-[#faf9f6] py-20">
      <div className="mx-auto grid max-w-[1320px] gap-px bg-[var(--charcoal)]/10 px-6 md:grid-cols-3 md:px-8">
        {QUICK_INFO.map((item) => (
          <a
            key={item.label}
            href={item.href}
          ...
          >
            <div className="flex items-center justify-between">
              <item.icon className="h-7 w-7 text-[var(--brand-orange)]" strokeWidth={1.4} />
              <ArrowUpRight className="h-5 w-5 text-[var(--charcoal)]/40 transition group-hover:translate-x-1 group-hover:text-[var(--brand-orange)]" />
            </div>

            <div className="mt-10 text-[10px] font-semibold uppercase tracking-[0.28em] text-[var(--charcoal)]/45">
              {item.label}
            </div>

            <div className="mt-3 text-[28px] font-light leading-tight text-[var(--ink)]">
              {item.value}
            </div>
          </a>
        ))}
...
function EnterpriseInquiry() {
  return (
    <section id="inquiry" className="border-t border-[var(--charcoal)]/10 bg-white py-24 md:py-32">
      <div className="mx-auto grid max-w-[1320px] gap-14 px-6 md:grid-cols-12 md:px-8">
        <div className="md:col-span-5">
          <div className="sticky top-24">
            ...
            <h2 className="text-[54px] leading-[0.98] tracking-tight text-[var(--ink)] md:text-[72px]">
        ...
        <div className="md:col-span-7">
          <form className="grid gap-5">
            ...
            <div>
              <label className="mb-3 block text-[10px] font-semibold uppercase tracking-[0.25em] text-[var(--charcoal)]/45">
                Nhu cầu quan tâm
              </label>
              <div className="grid gap-3 md:grid-cols-3">
                {INTERESTS.map((item) => (
                  <label
                    key={item}
                    className="flex cursor-pointer items-center gap-3 border border-[var(--charcoal)]/10 bg-[#faf9f6] px-4 py-3 text-[13px] text-[var(--charcoal)]/75 transition hover:border-[var(--brand-orange)] hover:bg-white"
                  >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full resize-none border border-[var(--charcoal)]/10 bg-[#faf9f6] px-5 py-4 text-[15px] leading-relaxed outline-none transition placeholder:text-[var(--charcoal)]/35 focus:border-[var(--brand-orange)] focus:bg-white"
              />
            </div>

            <button
              type="submit"
              className="mt-3 inline-flex w-fit items-center gap-2 rounded-full bg-[var(--brand-red)] hover:bg-[var(--brand-orange)] px-8 py-4 text-[12px] font-semibold uppercase tracking-[0.18em] text-white transition hover:brightness-110"
            >
              Gửi yêu cầu tư vấn
...
function OfficeNetwork() {
  return (
    <section
      id="office-network"
      className="border-t border-[var(--charcoal)]/10 bg-[#faf9f6] py-24 md:py-32"
    >
      <div className="mx-auto max-w-[1320px] px-6 md:px-8">
        ...
        <div className="grid gap-px bg-[var(--charcoal)]/10 md:grid-cols-2 lg:grid-cols-5">
          {OFFICES.map((office) => (
            <article key={office.city} className="bg-[#faf9f6] p-7 transition hover:bg-white">
              ...
              <h3 className="mt-8 text-[30px] font-light leading-tight text-[var(--ink)]">
                {office.city}
              </h3>
...
function BusinessUnits() {
  return (
    <section className="border-t border-[var(--charcoal)]/10 bg-white py-24 md:py-32">
      <div className="mx-auto grid max-w-[1320px] gap-14 px-6 md:grid-cols-12 md:px-8">
        <div className="md:col-span-5">
          <div className="sticky top-24">
        ...
        <div className="md:col-span-7">
          <div className="border-t border-[var(--charcoal)]/10">
            {BUSINESS_UNITS.map((unit, index) => (
              <article
                key={unit.title}
                className="group grid gap-6 border-b border-[var(--charcoal)]/10 py-8 md:grid-cols-[90px_1fr]"
              >
                ...
                <div>
                  <div className="flex items-start justify-between gap-6">
                    <h3 className="text-[32px] font-light leading-[1.1] tracking-[-0.03em] text-[var(--ink)] transition group-hover:text-[var(--brand-orange)]">
                      {unit.title}
                    </h3>
                    <ArrowUpRight className="mt-2 h-5 w-5 text-[var(--charcoal)]/40 transition group-hover:translate-x-1 group-hover:text-[var(--brand-orange)]" />
                  </div>

                  <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-[var(--charcoal)]/75">
                    {unit.desc}
                  </p>
                </div>
              </article>
...
function Field({
  ...
  return (
    <label>
      <span className="mb-3 block text-[10px] font-semibold uppercase tracking-[0.25em] text-[var(--charcoal)]/45">
        {label}
      </span>
      <input
        type={type}
        placeholder={placeholder}
        className="w-full border border-[var(--charcoal)]/10 bg-[#faf9f6] px-5 py-4 text-[15px] outline-none transition placeholder:text-[var(--charcoal)]/35 focus:border-[var(--brand-orange)] focus:bg-white"
      />
    </label>
  );
}
    ...
    <div className="mb-6 flex items-center gap-3">
      ...
      <span
        className={`text-[11px] font-semibold uppercase tracking-[0.3em] ${
          dark ? "text-white/70" : "text-[var(--charcoal)]/60"
        }`}
      >
        {children}
      </span>
