      ...
      <div className="mx-auto max-w-[1320px] px-8">
        ...
        <div className="mt-14 border border-[var(--border)] bg-white shadow-[0_30px_80px_-40px_rgba(0,0,0,0.25)]">
          ...
          <div className="relative flex items-center justify-between border-b border-[var(--border)] bg-[var(--ink)] px-7 py-3 text-white">
            ...
            {/* Connector line dropping toward active company */}
            <span
              aria-hidden
              className="pointer-events-none absolute bottom-0 h-3 w-px translate-y-full bg-[var(--brand-orange)] transition-all duration-500"
              style={{
            ...
            {MEMBERS.map((mem, i) => {
              ...
              return (
                <button
                  key={mem.id}
                  onClick={() => setActive(i)}
                  className={`relative flex flex-col justify-center items-start gap-1 border-b border-r border-[var(--border)] px-4 h-[66px] w-full text-left transition ${
                    isActive ? "bg-white" : "bg-[var(--warm-white)] hover:bg-white"
                  }`}
                >
                  <span
                    className={`text-[10px] font-semibold tracking-[0.28em] ${
                      isActive ? "text-[var(--brand-orange)]" : "text-[var(--charcoal)]/45"
                    }`}
                  >
                    {mem.index}
                  </span>
                  ...
                  <span
                    className={`text-[16px] leading-tight ${
                      isActive ? "text-[var(--brand-orange)]" : "text-[var(--ink)]"
                    }`}
                  >
                    {mem.name}
                  </span>
                  ...
                  {isActive && (
                    <span className="absolute -bottom-px left-0 right-0 h-[2px] bg-[var(--brand-orange)]" />
                  )}
                </button>
              );
            })}
            ...
            <div className="flex flex-col justify-between gap-8 border-t border-[var(--border)] bg-[var(--warm-white)] p-10 lg:border-l lg:border-t-0">
              <div>
                <div className="flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.28em] text-[var(--charcoal)]/65">
                  <span className="h-px w-6 bg-[var(--brand-orange)]" />
                ...
                <div className="mt-3 text-[12px] font-semibold uppercase tracking-[0.22em] text-[var(--brand-orange)]">
                  {m.tag}
                </div>
              ...
              <div className="flex items-end justify-between">
                <a
                  href="#"
                  className="group inline-flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.22em] text-[var(--ink)] hover:text-[var(--brand-orange)] transition-colors"
                >
                  Truy cập website
