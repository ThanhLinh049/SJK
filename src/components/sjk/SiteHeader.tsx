    ...
    <header className="relative z-50">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-6 md:px-8 md:py-7">
        <a href="/" className="relative z-50 flex items-center gap-2">
          <span className="text-2xl tracking-tight text-white">SJK</span>
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className="relative z-50 inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white backdrop-blur transition hover:border-[var(--brand-red)] md:hidden"
        >
        ...
        <div className="flex min-h-screen flex-col px-6 pb-8 pt-28">
          <nav className="border-t border-white/10">
            {NAV.map((n, index) => (
              ...
              >
                <div>
                  <div className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[var(--brand-red)]">
                ...
                <span className="text-2xl text-white/30 transition group-hover:translate-x-1 group-hover:text-[var(--brand-red)]">
                  →
                </span>
              </a>
            ))}
          ...
          <div className="mt-auto border-t border-white/10 pt-6">
            <a
              href="/contact"
              onClick={() => setOpen(false)}
              className="inline-flex w-full items-center justify-center rounded-full bg-[var(--brand-red)] px-6 py-4 text-[12px] font-semibold uppercase tracking-[0.18em] text-white transition hover:brightness-110"
            >
              Liên hệ hợp tác
            </a>

            <div className="mt-6 grid grid-cols-2 gap-4 text-[12px] text-white/60">
              <a href="tel:0933159988" className="hover:text-white">
                0933 15 99 88
              </a>
              <a href="mailto:info@sjkgroup.vn" className="text-right hover:text-white">
                info@sjkgroup.vn
              </a>
            </div>
